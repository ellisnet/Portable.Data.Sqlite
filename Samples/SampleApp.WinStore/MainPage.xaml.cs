            ...
            try {
                ...
                using (var dbConn = new SqliteAdoConnection(myApp.SqliteConnection, myApp.AppCryptEngine)) {
                    ...
                    using (var cmd = new SqliteCommand(sql, dbConn)) {
                        ...
                        Debug.WriteLine("A record with ID " + newRowId.ToString() + " was created in table [" + myTableName + "].");
                    }
                    ...
                    using (var cmd = new SqliteCommand(sql, dbConn)) {
                        ...
                        Debug.WriteLine("The oldest record in table [" + myTableName + "] has timestamp: " + oldest.ToString());
                    }
                ...
                myMessage = new MessageDialog("If you can see this message, then the sample Portable.Data.Sqlite code ran correctly. " +
                    "Take a look at the code in the 'MainPage.xaml.cs' file, and compare it to what you are seeing in the IDE Output window. " +
                    "And have a nice day...",
                    "All Done! - Check the Output window");
                myMessage.Commands.Add(new UICommand("OK"));
                myMessage.DefaultCommandIndex = 0;              

            }
            catch (Exception ex) {
                myMessage = new MessageDialog("Error: " + ex.Message, "An Error Occurred");
                myMessage.Commands.Add(new UICommand("OK"));
                myMessage.DefaultCommandIndex = 0;                
            }
